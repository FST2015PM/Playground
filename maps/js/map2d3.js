...
function createMap(){
  var  archivo = document.getElementById('csv').value;
       divmapa = document.getElementById('mapa').value;
       divmsg = document.getElementById('msg').value;
alert(divmapa);
  var width = 960,
      height = 628;
  ...
  var quantize = d3.scaleQuantize()
    ...
    var svg = d3.select(document.getElementById(divmapa)).append("svg")
      .attr("width", width)
      .attr("height", height);

    queue()
      .defer(d3.csv, archivo.concat(".csv"), function(d) { rateById.set(d.id, +d.rate); })
   ...
   var h1 = d3.select(document.getElementById(divmsg)).append("h1").text("texto adicional")
